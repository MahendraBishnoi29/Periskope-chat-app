...
export default function LabelFilter({ onFilterChange, selectedLabelIds }: LabelFilterProps) {
  ...
  useEffect(() => {
    const fetchLabels = async () => {
      ...
      try {
        ...
        const { data: newSchemaData, error: newSchemaError } = await supabase.from("labels").select("*")
        ...
        if (!newSchemaError && newSchemaData && newSchemaData.length > 0) {
          ...
          if (!oldSchemaError && oldSchemaData) {
            ...
            const uniqueLabels = [...new Set(oldSchemaData.map((item: any) => item.label))]
            setLabels(
  ...
  return (
    <div className="flex items-center">
        ...
        <PopoverTrigger asChild>
          <Button
            ...
            className={cn(
              ...
              selectedLabelIds.length > 0 && "bg-blue-50 border-blue-200 text-blue-600",
            )}
          >
            <Filter size={14} />
            <span>{selectedLabelIds.length > 0 ? `Filtered (${selectedLabelIds.length})` : "Filter by Label"}</span>
          </Button>
        </PopoverTrigger>
                  ...
                  labels.map((label) => (
                    <div key={label.id} className="flex items-center">
                      ...
                      >
                        ...
                        />
                        <span className={cn("px-2 py-0.5 text-xs rounded-md", label.color)}>{label.name}</span>
                      </label>
                    </div>
                  ))
                )}
      ...
      {selectedLabelIds.length > 0 && (
        <Button variant="ghost" size="sm" onClick={clearFilters} className="ml-1 h-8 px-2">
          <X size={14} />
        </Button>
      )}
    </div>
